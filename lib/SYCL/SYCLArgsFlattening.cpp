...
//===----------------------------------------------------------------------===//
//
// Transforms the arguments given to a SYCL kernel (typically a lambda capture
// passed by address) by a flatten version, more suitable to argument
// serialization by OpenCL clSetKernelArg() for example.
//
//
// This pass is basically an adaptation from
// lib/Transforms/IPO/ArgumentPromotion.cpp and a huge factorization should be
// done.
//
...
/// SYCLArgsFlattening - The 'by reference' to 'by value' argument promotion
/// pass.
struct SYCLArgsFlattening : public CallGraphSCCPass {
  // Pass identification, replacement for typeid
  static char ID;

  // Allows infinite elements to be flatten
...
bool SYCLArgsFlattening::runOnSCC(CallGraphSCC &SCC) {
  if (skipSCC(SCC))
    return false;
...
bool SYCLArgsFlattening::doInitialization(CallGraph &CG) {
